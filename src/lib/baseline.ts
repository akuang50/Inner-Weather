...
import type { DaySnapshot, HealthLog, JournalEntry, LanguageAnalysis } from './trackerTypes'
...
export function computeStressScore(body: number, language: number, context: number) {
...
export function buildDaySeries(healthLogs: HealthLog[], journals: JournalEntry[], days = 7): DaySnapshot[] {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const score = computeStressScore(body.score, lang.score, ctx)
    const coOccurrence = body.score >= 0.45 && lang.score >= 0.45
