...
export function SignalExplorer() {
  const { body, language, coOccurrence, stressScore, messagingScore, latestTone } = useTracker()
...
function WordsPanel() {
  const { language, messagingScore, latestTone } = useTracker()
  ...
  return (
    <div className="grid gap-10 md:grid-cols-2">
      <div className="space-y-5">
        ...
        <LangRow label="Message tone" value={Math.round(messagingScore * 100)} />
        {latestTone?.flags?.[0] && (
          <p className="text-sm text-muted">{latestTone.flags[0]}</p>
        )}
      </div>
      <div>
      ...
      <div className="grid gap-8 md:grid-cols-[0.9fr_1.1fr] md:items-center">
        ...
        <div>
          ...
          <ul className="mt-6 space-y-3 text-white/75">
            ...
            <li>Message tone signal is at {Math.round(messagingScore * 100)}.</li>
            {toneFlag && <li>{toneFlag}.</li>}
            {language.topics[0] && (
              <li>
